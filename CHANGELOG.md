...
- `jms/translation-bundle` is removed. See README.md for up-to-date
instructions on updating the translations.
- The deploy user credentials are removed. Gateway does not change the
schema, and the credentials are cleaned up.
...
- Ensure the `database_server_version` setting in parameters.yaml is
up-to-date with the production db server version. e.g. `10.6.23-MariaDB`

Attention:

- `jms/translation-bundle` seems EOL, it relies on `doctrine/annotations`,
which is abandoned. See <https://github.com/OpenConext/Stepup-Gateway/issues/471>

## 5.1.1

- Bugfix: LoA 1.5 SFO authentications fail for institutions with default
config configuration (SSO & GSSP fallbang off) when GSSP fallback attributes
are present ()
...
- Bugfix: set the correct response Twig template when rendering the ADFS
response #333
- Enable part of the behat test suite #333 Also see:
<https://www.pivotaltracker.com/story/show/187940465>

## 5.0.1

- Bugfix: implode was called with the wrong argument sequence. Which would
work in PHP<8.0 #332
...
- After fixing error response handling in 4.1.0, the possibility of handling a
failing ADFS response was not yet implemented. And that was taken care of in
this release. See #285 for details.
...
- Repaired a problem with the SAML error response handling when cancelling a
GSSP registration #283
...
- Downgrade Stepp-bundle (revert of #238) as this required us to install too
much untested stepup-saml-bundle features.

## 4.0.1

Add support for the new identity-vetting setup for SAT second factor tokens.
In addition two important security updates where installed. For additional
details check these closed PR's
<https://github.com/OpenConext/Stepup-Gateway/pull/277> and
<https://github.com/OpenConext/Stepup-Gateway/pull/276>.
...
Restore simultaneous SFO and SSO authentications feature. This was accidently
removed in version 3.2.0 Resolved issues with naming
...
- Ensure SFO SAML errors are handled correctly. In version 3.0 they yielded
internal server errors, as the application tried to send the response back
using the SSO authentication session context.
...
- Allow simultaneous SFO and SSO authentications. To do this the state handling
was changed slightly. Some new routes where added to distinguish between sfo
and sso authentications in Gateway. That's why a new major release was tagged.
...
- The previously hardcoded "server_version" config option (Doctrine DBAL) is
now configurable
